...
export default {
  ...
  async TAG_ARTICLES({ commit, state }, params) {
    const { data } = await axios.get(
      `${state.app.baseUrl}/tag/${params.id}/${state.limit}/${params.page}`
    )
  ...
  async ARTICLES({ commit, state }, page) {
    const { data } = await axios.get(
      `${state.app.baseUrl}/articles/${state.limit}/${page}`
    )
  ...
  async STICK_ARTICLES({ commit, state }, page) {
    const { data } = await axios.get(
      `${state.app.baseUrl}/stick/${state.limit}/${page}`
    )
  ...
  async DRAFTS({ commit, state }, page) {
    const { data } = await axios.get(
      `${state.app.baseUrl}/drafts/${state.limit}/${page}`
    )
  ...
  async SEARCH({ commit, state }, params) {
    const { data } = await axios.get(
      `${state.app.baseUrl}/search/${params.id}/${state.limit}/${params.page}`
    )
